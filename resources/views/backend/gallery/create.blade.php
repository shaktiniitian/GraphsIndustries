...
<div class="content-wrapper">
                    ...
                    <div class="ibox-body">
                        ...
                        <div class="form-group mb-4">
                                ...
                                <select name="type" class="form-control">
                                    ...
                                    @foreach('\App\Category'::where('type','=','gallery')->get() as $type)
                                    <option
                                        {{ !empty($item) && $item->category_id==$type->id ? 'selected' : ($type==old('type') ? 'selected':'') }}
 ...
 @if(Session::get('galleryType'))
                            <!-- <div class="form-group mb-4">
                                <label class="ui-switch switch-icon switch-solid-success mr-3 mb-0">
                                    <input type="checkbox" name="is_brochure" value="{{ Session::get('galleryType') }}"
                                        {{ !empty($item) ? ($item->type ? 'checked' : '') : '' }}>
                                    <span></span>
                                </label>Make It {{ Session::get('galleryType') }}  
                            </div> -->
@endif
