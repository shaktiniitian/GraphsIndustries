...
@section('content')
@php
$action = !empty($item) ? route('projects.update', $item->id) : route('projects.store');
@endphp
                        ...
                        <form action="{{ $action }}" method="POST" enctype="multipart/form-data">
                            ...
                            @endif
                                ...
                                <select name="type" class="form-control">
                                    ...
                                    @foreach([['id'=>11, 'title'=>'Message'],['id'=>13, 'title'=>'Message'],['id'=>12, 'title'=>'Message']] as $type)
                                    <option
                                        {{ !empty($item) && $item->type==$type['id'] ? 'selected' : ($type['id']==old('type') ? 'selected':'') }}
                         ...
                         <div class="form-group mb-4">
                                ...
                                <textarea rows="4" class="form-control" name="short_description"
                                    placeholder="Enter Short Description">{{ !empty($item) ? $item->datas : old('short_description') }}</textarea>
                            </div>
